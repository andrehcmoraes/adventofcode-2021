...
void numbers_add(numbers_t **n, stack_t *t) {
  t->next = (*n)->s;
  t->prev = NULL;
  if((*n)->s != NULL) (*n)->s->prev = t;
  (*n)->s = t;
  (*n)->len++;
  for(int i=0; i<NUM_BITS; i++)
    if(t->bin[i] == '1') (*n)->ones_sum[i]++;
}

// Split nodes into two lists, doesn't reallocate anything
void numbers_split(numbers_t **nums, numbers_t **oxy_nums, numbers_t **co2_nums) {
  char c = 2*(*nums)->ones_sum[0] >= (*nums)->len ? '1' : '0';
  while((*nums)->s != NULL) {
    stack_t *t = (*nums)->s;
    (*nums)->s = (*nums)->s->next;
    if(t->bin[0] == c)
      numbers_add(oxy_nums, t);
    else 
      numbers_add(co2_nums, t);
  }
}
...
int main() {
  ...
  while(fscanf(stdin, format, buff) > 0) {
    nums->len++;
    for(int i=0; i<NUM_BITS; i++)
      if(buff[i] == '1') nums->ones_sum[i]++;
    stack_append(&(nums->s), buff);
  }
  
  // Split numbers in two lists
  numbers_init(&oxy_nums);
  numbers_init(&co2_nums);
  numbers_split(&nums, &oxy_nums, &co2_nums);
