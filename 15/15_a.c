...
uint64_t find_path(char **grid, uint64_t ncols, uint64_t nrows,
  ...
  while(paths != NULL) {
    ...
    // Queue neighbours
    // Since all edges to a node have the same weight 
    // We can set a node as visited as soon as it is queued
