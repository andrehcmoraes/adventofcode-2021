...
unsigned long long grid_fold(char **grid, int *lx, int *ly, char c, int n) {
...
int main() {
  ...
  // Build grid
  char **grid;
  grid = (char **)malloc(len_y * sizeof(*grid));
  for(int i=0; i<len_y; i++) {
    grid[i] = (char *)malloc(len_x * sizeof(*(grid[i])));
